"""
Ini adalah file klien agen. Klien ini bertanggung jawab untuk berinteraksi dengan pengguna melalui chat. Klien ini menggunakan model bahasa untuk menghasilkan respons yang sesuai berdasarkan input pengguna.
"""
...
from config.config import GROQ_API_KEY, MAX_TOKENS, MODEL_NAME, TEMPERATURE
...
class StateSchema(TypedDict):
    """
    Schema untuk menyimpan status dari sesi chat. Ini termasuk daftar pesan,
    ringkasan, ID guild, ID thread, dan ID pengguna.
    """
...
class AgentClient:
    """
    Kelas ini mewakili klien agen yang berinteraksi dengan pengguna melalui chat. Klien ini menggunakan model bahasa untuk menghasilkan respons yang sesuai berdasarkan input pengguna.
    """
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initializing GroqClient with model: {model_name}, temprature: {temperature}, max_token: {max_token}"
        )
    ...
    async def __async_call_llm_node(self, state: StateSchema):
        ...
        log_msg = {
            ...
            "time_usage": {
                "completion_time": response.response_metadata["token_usage"][
                    "completion_time"
                ],
    ...
    async def invoke(
        ...
        try:
            ...
            knowledges: List[tuple[Document, float]] = self.retriever.loadData(
            ...
            initial_state: StateSchema = {
                "messages": [
                    SystemMessage(
                        (
                            system_message
                            or "Kamu adalah asisten AI yang membantu menjawab pertanyaan.\n"
                        )
                        + ("\nUser context:\n")
                        # + (f"- guild_id: {guild_id}\n")
