...
class DiscordBot(Bot):
            ...
            try:
                ...
                if response_get_data.data:
                    await interaction.followup.send(
                        f"Previous response: {response_get_data.data}"
                    )
                ...
                data_payload = {
                    "issue_category_id": (
                        ...
                        else old_data["data"]["issue_category_id"] or None
                    ),
                }
                payload = {
                    "guild_id": guild_id,
                    "data": data_payload,
                    "update_at": (
                        datetime.now().isoformat() if response_get_data.data else None
                    ),
                }
                if response_get_data.data:
                ...
                await interaction.followup.send(
                    f"Success save data with result: {response_create_data.data or "Success"}"
                )
            except Exception as e:
                await interaction.followup.send(
                    f"Maaf, terjadi kesalahan saat mengatur ID kategori masalah:\n{e}"
                )
                self.logger.error(
                    f"Terjadi kesalahan saat mengatur ID kategori masalah: {str(e)}"
                )
                self.logger.exception("Detail error:")
    ...
    async def on_ready(self):
        ...
        self.logger.info(
            f"Bot is ready with {len(self.ai_agent.tools)} available tools"
        )
